...
class SegmentationLosses(object):
    ...
    def BCELoss(self, logit, target):
        ...
        if self.cuda:
            criterion = criterion.cuda()
        loss = criterion(logit, true_1_hot)
        return loss
    ...
    def BALoss(self, logit, target, remain_rate=0.5, edge_multiplier=1.5):
        ...
        fe = F.conv2d(target, edge_filter, stride=1, padding=0)
        eg = torch.ge(fe,0.).float()
        filter_size = np.array([35, 35])
        gaussian_edge = self.gaussianize_image(eg, filter_size)
        gaussian_edge = F.interpolate(gaussian_edge, size=target.size()[2:], mode='bilinear', align_corners=True)
